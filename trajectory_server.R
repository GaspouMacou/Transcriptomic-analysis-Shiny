...
trajectory_server <-  function(input, output, session) {
    ...
    # Tab 19
    seurat_monocle <- reactiveVal() # Your Seurat object reactive container
    monocle_object <- reactiveVal() # Your Monocle object reactive container
    ...
    observeEvent(input$diffGeneTestButton, {
      ...
      # Calculer et stocker les résultats des tests de gènes différentiels
      diff_gene_results_temp <- graph_test(monocle_object(), neighbor_graph = "principal_graph", cores = 4)

      # Mettre à jour la valeur réactive avec les résultats du test
      pseudotime_diff_gene_results(diff_gene_results_temp)

      # Mettre à jour le tableau de résultats dans l'UI
      output$diffGeneTable <- DT::renderDataTable({
        DT::datatable(subset(diff_gene_results_temp, q_value < 0.05))
      })
    })
